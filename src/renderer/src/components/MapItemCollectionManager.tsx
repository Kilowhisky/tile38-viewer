...
export function MapItemCollectionManager() {
  ...
  return <>
    ...
    >{itemCollections.size} Collections</Button>
    <Popover
      anchorEl={anchorEl}
      open={open}
      onClose={handleClose}
    ...
    > {collections.map(collection => (
      <div key={collection.id} className="collection">
        <span className="collection-color"><ColorPicker color={collection.color} onChange={c => colorChange(collection, c)} /></span>
        ...
        <span className="collection-delete">
          <IconButton
            ...
            size="small"
            title="Remove"
            onClick={() => removeItemCollection(collection.id)}
