...
import { KeyStats } from "@/lib/tile38Connection.models"
    ...
    <div className="keys-container">
      {keys.slice(0, keyCountShown).map(k => (
        ...
        >
          ...
          <AccordionDetails className="key-details">
            <KeyItemList itemKey={k.key} />
          </AccordionDetails>
        </Accordion>
      ))}
      {moreToShow && (
        <Button fullWidth className="load-more" variant="contained" sx={{ borderRadius: 0 }} onClick={() => setKeyCountShown(keyCountShown + 50)}>
          Load 50 more...
        </Button>
      )}
    </div>
      ...
      <div className="chip-container">
        ...
        <div className="chip-block">
          <StatChip enabled={true} title="Memory Used">
            <span>
              <MemoryIcon />
              {!stats ? <CircularProgress size="0.75em" /> : <HumanFileSize size={stats?.in_memory_size || 0} />}
            </span>
          </StatChip>
          <StatChip enabled={true} title="Count of Items in Key">
            <span>
              <NumbersIcon />
              {count || <CircularProgress size="0.75em" />}
            </span>
          </StatChip>
        </div>
      </div>
...
export function KeysMenu() {
  ...
  return (
    <div>
      ...
      <Menu id="basic-menu" variant="menu" anchorEl={anchor} open={anchor !== undefined} onClose={() => setAnchor(undefined)} onClick={e => e.stopPropagation()}>
        <MenuItem>
          <TextField
            label="MATCH"
            variant="standard"
